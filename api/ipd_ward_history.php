...
try {
    ...
    if ($fiscalYear < 2500 || $fiscalYear > 2600) {
        $fiscalYear = ((int)date('Y') + 543) + (date('n') >= 10 ? 1 : 0);
    }

    $sql = "WITH params AS (
                SELECT
                    :ward::varchar AS ward_code,
            ...
            monthly_admit AS (
                ...
                GROUP BY date_trunc('month', i.regdate)::date
            ),
                ...
                SELECT m.month_start,
                       SUM(
                           GREATEST(
                               ...
                               (
                                   LEAST(
                                       COALESCE(i.dchdate::date, m.month_start + INTERVAL '1 month'),
            ...
            SELECT
                ...
                END AS fiscal_year,
                TO_CHAR(m.month_start, 'TMMonth') AS month_name,
                wi.ward,
                wi.ward_name,
                wi.bedcount,
                ...
                ROUND(
                    ...
                    / NULLIF(
                        wi.bedcount * ((m.month_start + INTERVAL '1 month')::date - m.month_start),
                        0
                    ),
                    2
                ) AS occupancy_rate
